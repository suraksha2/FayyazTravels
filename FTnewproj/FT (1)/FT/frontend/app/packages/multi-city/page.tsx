...
const getImageUrl = (imagePath: string) => {
  if (!imagePath) return 'https://images.pexels.com/photos/2193300/pexels-photo-2193300.jpeg';
  if (imagePath.startsWith('http')) return imagePath;
  return `${API_BASE}/${imagePath}`;
}

export default function MultiCityPackagesPage() {
  const { isOpen, modalData, openModal, closeModal } = useEnquiryModal();
  const [multiCityPackages, setMultiCityPackages] = useState<any[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchPackages = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`${API_BASE}/packages/multi-city`, {
          signal: controller.signal
        });
        if (isMounted && response.data.success) {
          setMultiCityPackages(response.data.packages);
        }
      } catch (err) {
    ...
    <main className="min-h-screen bg-white">
      ...
      <div className="relative h-screen">
        ...
        <div className="relative h-full flex flex-col items-center justify-center text-white text-center px-4">
          ...
          <p className="text-2xl max-w-2xl mx-auto mb-8">
            Explore multiple destinations in one incredible journey with our multi-city packages.
          </p>
          <Button 
            onClick={() => openModal({
              packageName: "Multi City Travel Packages",
      ...
      <section className="py-16 px-4 max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <h2 className="text-4xl font-bold text-gray-900 mb-4">Multi City Adventures</h2>
          <p className="text-gray-600 text-lg">
            Discover exclusive offers on multi-destination travel packages.
          </p>
        </div>
        ...
        {!loading && !error && multiCityPackages.length === 0 && (
          <div className="text-center py-12">
