...
export default function BulgariaGreecePage() {
  ...
  useEffect(() => {
    const fetchPackages = async () => {
      try {
        const response = await fetch('http://localhost:3003/destination/multi-city/bulgaria-greece')
        if (!response.ok) {
    ...
    <main className="min-h-screen bg-white">
      ...
      <div className="relative h-screen">
        ...
        <div className="relative h-full flex flex-col items-center justify-center text-white text-center px-4">
          ...
          <p className="text-2xl max-w-2xl mx-auto mb-8">
            Discover ancient civilizations and stunning coastlines across the Balkans and Mediterranean.
          </p>
          <Button 
            onClick={() => openModal({
              packageName: "Bulgaria & Greece Travel Packages",
              packageType: "Multi-City Package",
              destination: "Bulgaria & Greece"
            })}
      ...
      <section className="py-16 px-4 max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <h2 className="text-4xl font-bold text-gray-900 mb-4">Bulgaria & Greece Packages</h2>
          <p className="text-gray-600 text-lg">
            Experience the best of Eastern European and Mediterranean cultures.
          </p>
        </div>
            ...
            <div key={pkg.id} className="bg-white rounded-2xl overflow-hidden shadow-lg transition-all duration-300 hover:shadow-2xl group">
              <Link href={`/packages/${pkg.p_slug}`} className="block">
                ...
                <div className="relative h-56 overflow-hidden">
                  <img 
                    src={pkg.feature_img ? `http://localhost:3003/${pkg.feature_img}` : 'https://images.pexels.com/photos/1010657/pexels-photo-1010657.jpeg'} 
                    alt={pkg.p_name}
