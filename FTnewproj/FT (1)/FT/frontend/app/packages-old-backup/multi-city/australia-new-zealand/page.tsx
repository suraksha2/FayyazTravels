...
export default function AustraliaNewZealandPage() {
  ...
  useEffect(() => {
    const fetchPackages = async () => {
      try {
        const response = await fetch('http://localhost:3003/destination/multi-city/australia-new-zealand')
        if (!response.ok) {
  ...
  const extractPrice = (content: string, inclusions: string): number => {
    // Try to extract price from content first
    const contentPriceMatch = content.match(/\$(\d+(?:,\d{3})*)/);
    if (contentPriceMatch) {
      return parseInt(contentPriceMatch[1].replace(/,/g, ''));
    }
    
    // Try to extract from inclusions
    const inclusionsPriceMatch = inclusions.match(/\$(\d+(?:,\d{3})*)/);
    if (inclusionsPriceMatch) {
      return parseInt(inclusionsPriceMatch[1].replace(/,/g, ''));
    }
    
    // Default prices based on duration
    const dayMatch = content.match(/(\d+)D/);
    const days = dayMatch ? parseInt(dayMatch[1]) : 7;
    return days >= 10 ? 4999 : days >= 7 ? 3999 : 2999;
  }
  ...
  // Helper function to get destination count
  const getDestinationInfo = (desti_list: string): string => {
    if (!desti_list) return 'Multiple Cities';
    const cities = desti_list.split(',').filter(city => city.trim());
    return `${cities.length} Cities`;
  }

  // Helper function to get tour highlights
  const getTourHighlights = (inclusions: string): string[] => {
    if (!inclusions) return [];
    const cleaned = inclusions.replace(/<[^>]*>/g, '').replace(/\r\n/g, ' ');
    const highlights = cleaned.split(/[-•]/).filter(item => item.trim() && item.length > 10).slice(0, 2);
    return highlights.map(h => h.trim().substring(0, 50) + (h.length > 50 ? '...' : ''));
  }
    ...
    <main className="min-h-screen bg-white">
      ...
      <div className="relative h-screen">
        <div 
          ...
          style={{
            backgroundImage: "url('https://images.pexels.com/photos/2193300/pexels-photo-2193300.jpeg')"
          }}
        >
        ...
        <div className="relative h-full flex flex-col items-center justify-center text-white text-center px-4">
          ...
          <p className="text-2xl max-w-2xl mx-auto mb-8">
            Discover two incredible countries in one unforgettable journey across the Pacific.
          </p>
          <Button 
            onClick={() => openModal({
              packageName: "Australia & New Zealand Travel Packages",
      ...
      <section className="py-16 px-4 max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <h2 className="text-4xl font-bold text-gray-900 mb-4">Australia & New Zealand Packages</h2>
          <p className="text-gray-600 text-lg">
            Experience the best of both countries with our carefully curated multi-city tours.
          </p>
        </div>
                ...
                <div className="p-5">
                  ...
                  <div className="flex items-center gap-3 mb-4 flex-wrap">
                    ...
                    <div className="flex items-center gap-1.5 text-sm text-gray-700">
                      ...
                      <span className="font-medium">{getDestinationInfo(pkg.desti_list)}</span>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex items-baseline gap-1">
                        <span className="text-3xl font-bold text-gray-900">
                          S${extractPrice(pkg.p_content, pkg.inclusions).toLocaleString()}
                        </span>
                      </div>
