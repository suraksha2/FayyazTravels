...
const getImageUrl = (imagePath: string) => {
  if (!imagePath) return 'https://images.pexels.com/photos/1878293/pexels-photo-1878293.jpeg';
  if (imagePath.startsWith('http')) return imagePath;
  return `${API_BASE}/${imagePath}`;
}

export default function OceaniaPackagesPage() {
  const { isOpen, modalData, openModal, closeModal } = useEnquiryModal();
  const [oceaniaPackages, setOceaniaPackages] = useState<any[]>([]);
  ...
  useEffect(() => {
    const fetchPackages = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`${API_BASE}/packages/oceania`);
        if (response.data.success) {
          setOceaniaPackages(response.data.packages);
        }
      } catch (err) {
    ...
    <main className="min-h-screen bg-white">
      ...
      <div className="relative h-screen">
        <div 
          ...
          style={{
            backgroundImage: "url('https://images.pexels.com/photos/1878293/pexels-photo-1878293.jpeg')"
          }}
        >
        ...
        <div className="relative h-full flex flex-col items-center justify-center text-white text-center px-4">
          ...
          <p className="text-2xl max-w-2xl mx-auto mb-8">
            Discover stunning landscapes, unique wildlife, and island paradise.
          </p>
          <Button
            onClick={() => openModal({
              packageName: "Oceania Travel Packages",
              packageType: "Oceania Package",
              destination: "Oceania"
            })}
      ...
      <section className="py-16 px-4 max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <h2 className="text-4xl font-bold text-gray-900 mb-4">Discover Oceania</h2>
          <p className="text-gray-600 text-lg">
            From Sydney's Opera House to New Zealand's fjords, explore the Pacific wonders.
          </p>
        </div>
        ...
        {!loading && !error && oceaniaPackages.length === 0 && (
          <div className="text-center py-12">
