...
const getImageUrl = (imagePath: string) => {
  if (!imagePath) return 'https://images.pexels.com/photos/1268855/pexels-photo-1268855.jpeg';
  if (imagePath.startsWith('http')) return imagePath;
  return `${API_BASE}/${imagePath}`;
}

export default function CaribbeanPackagesPage() {
  const { isOpen, modalData, openModal, closeModal } = useEnquiryModal();
  const [caribbeanPackages, setCaribbeanPackages] = useState<any[]>([]);
  ...
  useEffect(() => {
    const fetchPackages = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`${API_BASE}/packages/the-caribbean`);
        if (response.data.success) {
          setCaribbeanPackages(response.data.packages);
        }
      } catch (err) {
    ...
    <main className="min-h-screen bg-white">
      ...
      <div className="relative h-screen">
        ...
        <div className="relative h-full flex flex-col items-center justify-center text-white text-center px-4">
          ...
          <p className="text-2xl max-w-2xl mx-auto mb-8">
            Tropical paradise and pristine beaches await your arrival.
          </p>
          <Button 
            onClick={() => openModal({
              packageName: "The Caribbean Travel Packages",
      ...
      <section className="py-16 px-4 max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <h2 className="text-4xl font-bold text-gray-900 mb-4">Discover The Caribbean</h2>
          <p className="text-gray-600 text-lg">
            From pristine beaches to vibrant island culture.
          </p>
        </div>
        ...
        {!loading && !error && caribbeanPackages.length === 0 && (
          <div className="text-center py-12">
