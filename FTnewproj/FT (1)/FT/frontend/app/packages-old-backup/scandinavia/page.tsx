...
const getImageUrl = (imagePath: string) => {
  if (!imagePath) return 'https://images.pexels.com/photos/1285625/pexels-photo-1285625.jpeg';
  if (imagePath.startsWith('http')) return imagePath;
  return `${API_BASE}/${imagePath}`;
}

export default function ScandinaviaPackagesPage() {
  const { isOpen, modalData, openModal, closeModal } = useEnquiryModal();
  const [scandinaviaPackages, setScandinaviaPackages] = useState<any[]>([]);
  ...
  useEffect(() => {
    const fetchPackages = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`${API_BASE}/packages/scandinavia`);
        if (response.data.success) {
          setScandinaviaPackages(response.data.packages);
        }
      } catch (err) {
    ...
    <main className="min-h-screen bg-white">
      ...
      <div className="relative h-screen">
        ...
        <div className="relative h-full flex flex-col items-center justify-center text-white text-center px-4">
          ...
          <p className="text-2xl max-w-2xl mx-auto mb-8">
            Experience the Northern Lights, fjords, and Nordic charm.
          </p>
          <Button
            onClick={() => openModal({
              packageName: "Scandinavia Travel Packages",
              packageType: "Scandinavia Package",
      ...
      <section className="py-16 px-4 max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <h2 className="text-4xl font-bold text-gray-900 mb-4">Discover Scandinavia</h2>
          <p className="text-gray-600 text-lg">
            From Oslo's Viking heritage to Stockholm's archipelago, explore Nordic wonders.
          </p>
        </div>
        ...
        {!loading && !error && scandinaviaPackages.length === 0 && (
          <div className="text-center py-12">
