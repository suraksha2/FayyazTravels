...
import { Heart, Calendar, MapPin, Facebook, Instagram, Twitter, Linkedin, Youtube, Music, Clock, ThumbsUp, CreditCard, Shield } from "lucide-react"
...
const getImageUrl = (imagePath: string) => {
  if (!imagePath) return 'https://images.pexels.com/photos/1282315/pexels-photo-1282315.jpeg';
  if (imagePath.startsWith('http')) return imagePath;
  return `${API_BASE}/${imagePath}`;
}

export default function SouthEastAsiaPackagesPage() {
  const { isOpen, modalData, openModal, closeModal } = useEnquiryModal();
  const [southEastAsiaPackages, setSouthEastAsiaPackages] = useState<any[]>([]);
  ...
  useEffect(() => {
    const fetchPackages = async () => {
      try {
        setLoading(true);
        console.log('Fetching from:', `${API_BASE}/packages/south-east-asia`);
        const response = await axios.get(`${API_BASE}/packages/south-east-asia`);
        if (response.data.success) {
          setSouthEastAsiaPackages(response.data.packages);
        }
      } catch (err) {
    ...
    <main className="min-h-screen bg-white">
      ...
      <div className="relative h-screen">
        ...
        <div className="relative h-full flex flex-col items-center justify-center text-white text-center px-4">
          ...
          <p className="text-2xl max-w-2xl mx-auto mb-8">
            Tropical adventures and cultural diversity across Southeast Asia.
          </p>
          <Button
            onClick={() => openModal({
              packageName: "South East Asia Travel Packages",
              packageType: "South East Asia Package",
      ...
      <section className="py-16 px-4 max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <h2 className="text-4xl font-bold text-gray-900 mb-4">Discover South East Asia</h2>
          <p className="text-gray-600 text-lg">
            From tropical beaches to ancient temples.
          </p>
        </div>
        ...
        {!loading && !error && southEastAsiaPackages.length === 0 && (
          <div className="text-center py-12">
