...
export default function MaldivesPackagesPage() {
  ...
  useEffect(() => {
    const fetchPackages = async () => {
      try {
        const response = await fetch('http://localhost:3003/destination/south-east-asia/maldives');
        if (!response.ok) {
    ...
    <main className="min-h-screen bg-white">
      ...
      <div className="relative h-screen">
        ...
        <div className="relative h-full flex flex-col items-center justify-center text-white text-center px-4">
          ...
          <p className="text-2xl max-w-2xl mx-auto mb-8">
            Experience ultimate luxury in overwater villas and pristine tropical atolls.
          </p>
          <Button 
            className="bg-white text-black hover:bg-white/90 text-lg px-8 py-6"
            onClick={() => openModal({
              packageName: "Maldives Travel Package",
      ...
      <section className="py-16 px-4 max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <h2 className="text-4xl font-bold text-gray-900 mb-4">Discover Maldives</h2>
          <p className="text-gray-600 text-lg">
            From overwater villas to pristine atolls, experience tropical luxury.
          </p>
        </div>
        ...
        {packages.length === 0 ? (
          <div className="col-span-full text-center py-12">
            ...
            <p className="text-gray-600">We're currently updating our Maldives packages. Please check back soon!</p>
          </div>
        ) : (
              ...
              >
                <div className="relative h-52 w-full group">
                  <Link href={`/packages/${pkg.slug || pkg.p_slug || pkg.id}`}>
                    <Image
                      src={pkg.p_image ? `http://localhost:3003/${pkg.p_image}` : 'https://images.pexels.com/photos/1287460/pexels-photo-1287460.jpeg'}
                      alt={pkg.p_name}
                      fill
