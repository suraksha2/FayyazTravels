...
export default function MyanmarPackagesPage() {
  ...
  useEffect(() => {
    const fetchPackages = async () => {
      try {
        const response = await fetch('http://localhost:3003/destination/south-east-asia/myanmar');
        if (!response.ok) {
    ...
    <main className="min-h-screen bg-white">
      ...
      <div className="relative h-screen">
        ...
        <div className="relative h-full flex flex-col items-center justify-center text-white text-center px-4">
          ...
          <p className="text-2xl max-w-2xl mx-auto mb-8">
            Explore ancient temples, golden pagodas, and rich Buddhist heritage.
          </p>
          <Button 
            className="bg-white text-black hover:bg-white/90 text-lg px-8 py-6"
            onClick={() => openModal({
              packageName: "Myanmar Travel Package",
      ...
      <section className="py-16 px-4 max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <h2 className="text-4xl font-bold text-gray-900 mb-4">Discover Myanmar</h2>
          <p className="text-gray-600 text-lg">
            From Bagan's temples to Inle Lake's serenity, experience Myanmar's magic.
          </p>
        </div>

        {packages.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-gray-500">No packages found for Myanmar. Please check back later.</p>
          </div>
        ) : (
