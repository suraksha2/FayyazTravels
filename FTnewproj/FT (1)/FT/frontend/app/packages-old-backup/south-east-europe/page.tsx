...
const getImageUrl = (imagePath: string) => {
  if (!imagePath) return 'https://images.pexels.com/photos/2166559/pexels-photo-2166559.jpeg';
  if (imagePath.startsWith('http')) return imagePath;
  return `${API_BASE}/${imagePath}`;
}

export default function SouthEastEuropePackagesPage() {
  const { isOpen, modalData, openModal, closeModal } = useEnquiryModal();
  const [southEastEuropePackages, setSouthEastEuropePackages] = useState<any[]>([]);
  ...
  useEffect(() => {
    const fetchPackages = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`${API_BASE}/packages/south-east-europe`);
        if (response.data.success) {
          setSouthEastEuropePackages(response.data.packages);
        }
      } catch (err) {
    ...
    <main className="min-h-screen bg-white">
      ...
      <div className="relative h-screen">
        ...
        <div className="relative h-full flex flex-col items-center justify-center text-white text-center px-4">
          ...
          <p className="text-2xl max-w-2xl mx-auto mb-8">
            Discover ancient history, stunning coastlines, and vibrant cultures.
          </p>
          <Button
            onClick={() => openModal({
              packageName: "South East Europe Travel Packages",
              packageType: "South East Europe Package",
              destination: "South East Europe"
            })}
      ...
      <section className="py-16 px-4 max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <h2 className="text-4xl font-bold text-gray-900 mb-4">Discover South East Europe</h2>
          <p className="text-gray-600 text-lg">
            From ancient ruins to stunning coastlines and vibrant cities.
          </p>
        </div>
        ...
        {!loading && !error && southEastEuropePackages.length === 0 && (
          <div className="text-center py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {southEastEuropePackages.map((pkg) => (
