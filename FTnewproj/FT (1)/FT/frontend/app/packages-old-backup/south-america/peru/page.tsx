...
export default function PeruPackagesPage() {
  ...
  useEffect(() => {
    const fetchPackages = async () => {
      try {
        const response = await fetch('http://localhost:3003/destination/south-america/peru')
        if (!response.ok) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">Loading Peru packages...</p>
        </div>
      </div>
    )
  }
    ...
    <main className="min-h-screen bg-white">
      ...
      <div className="relative h-screen">
        ...
        <div className="relative h-full flex flex-col items-center justify-center text-white text-center px-4">
          ...
          <p className="text-2xl max-w-2xl mx-auto mb-8">
            Discover Peru's ancient mysteries from Machu Picchu to the Sacred Valley, where Inca heritage meets stunning Andean landscapes.
          </p>
          <Button 
            onClick={() => openModal({
              packageName: "Peru Travel Packages",
      ...
      <section className="py-16 px-4 max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <h2 className="text-4xl font-bold text-gray-900 mb-4">Peru Heritage Packages</h2>
          <p className="text-gray-600 text-lg">
            Journey through Peru's rich history and breathtaking landscapes, from ancient Inca ruins to vibrant colonial cities.
          </p>
        </div>
