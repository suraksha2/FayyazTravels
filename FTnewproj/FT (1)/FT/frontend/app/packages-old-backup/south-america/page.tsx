...
const getImageUrl = (imagePath: string) => {
  if (!imagePath) return 'https://images.pexels.com/photos/2166553/pexels-photo-2166553.jpeg';
  if (imagePath.startsWith('http')) return imagePath;
  return `${API_BASE}/${imagePath}`;
}

export default function SouthAmericaPackagesPage() {
  const { isOpen, modalData, openModal, closeModal } = useEnquiryModal();
  const [southAmericaPackages, setSouthAmericaPackages] = useState<any[]>([]);
  ...
  useEffect(() => {
    const fetchPackages = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`${API_BASE}/packages/south-america`);
        if (response.data.success) {
          setSouthAmericaPackages(response.data.packages);
        }
      } catch (err) {
    ...
    <main className="min-h-screen bg-white">
      ...
      <div className="relative h-screen">
        ...
        <div className="relative h-full flex flex-col items-center justify-center text-white text-center px-4">
          ...
          <p className="text-2xl max-w-2xl mx-auto mb-8">
            Ancient civilizations and natural wonders across South America.
          </p>
          <Button 
            onClick={() => openModal({
              packageName: "South America Travel Packages",
              packageType: "South America Package",
      ...
      <section className="py-16 px-4 max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <h2 className="text-4xl font-bold text-gray-900 mb-4">Discover South America</h2>
          <p className="text-gray-600 text-lg">
            From ancient ruins to vibrant cities.
          </p>
        </div>
        ...
        {!loading && !error && southAmericaPackages.length === 0 && (
          <div className="text-center py-12">
