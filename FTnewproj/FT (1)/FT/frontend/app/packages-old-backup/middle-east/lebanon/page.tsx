...
export default function LebanonPackagesPage() {
  ...
  useEffect(() => {
    const fetchPackages = async () => {
      try {
        const response = await fetch('http://localhost:3003/destination/middle-east/lebanon');
        if (!response.ok) {
    ...
    <main className="min-h-screen bg-white">
      ...
      <div className="relative h-screen">
        ...
        <div className="relative h-full flex flex-col items-center justify-center text-white text-center px-4">
          ...
          <p className="text-2xl max-w-2xl mx-auto mb-8">
            Discover the Paris of the Middle East with stunning mountains and Mediterranean coast.
          </p>
          <Button 
            className="bg-white text-black hover:bg-white/90 text-lg px-8 py-6"
            onClick={() => openModal({
              packageName: "Lebanon Travel Package",
      ...
      <section className="py-16 px-4 max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <h2 className="text-4xl font-bold text-gray-900 mb-4">Discover Lebanon</h2>
          <p className="text-gray-600 text-lg">
            From Beirut's vibrant culture to Byblos' ancient ruins, explore the jewel of the Mediterranean.
          </p>
        </div>

        {packages.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-gray-500">No packages found for Lebanon. Please check back later.</p>
          </div>
        ) : (
