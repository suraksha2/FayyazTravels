...
const getImageUrl = (imagePath: string) => {
  if (!imagePath) return 'https://images.pexels.com/photos/2363/pexels-photo-2363.jpeg';
  if (imagePath.startsWith('http')) return imagePath;
  return `${API_BASE}/${imagePath}`;
}

export default function EuropePackagesPage() {
  const { isOpen, modalData, openModal, closeModal } = useEnquiryModal();
  const [europePackages, setEuropePackages] = useState<any[]>([]);
  ...
  useEffect(() => {
    const fetchPackages = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`${API_BASE}/packages/europe`);
        if (response.data.success) {
          setEuropePackages(response.data.packages);
        }
      } catch (err) {
    ...
    <main className="min-h-screen bg-white">
      ...
      <div className="relative h-screen">
        <div 
          ...
          style={{
            backgroundImage: "url('https://images.pexels.com/photos/2363/pexels-photo-2363.jpeg')"
          }}
        >
        ...
        <div className="relative h-full flex flex-col items-center justify-center text-white text-center px-4">
          ...
          <p className="text-2xl max-w-2xl mx-auto mb-8">
            Explore iconic cities, rich history, and diverse cultures across Europe.
          </p>
          <Button
            onClick={() => openModal({
              packageName: "Europe Travel Packages",
              packageType: "Europe Package",
              destination: "Europe"
            })}
      ...
      <section className="py-16 px-4 max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <h2 className="text-4xl font-bold text-gray-900 mb-4">Discover Europe</h2>
          <p className="text-gray-600 text-lg">
            From the Eiffel Tower to the Colosseum, experience Europe's finest destinations.
          </p>
        </div>
        ...
        {!loading && !error && europePackages.length === 0 && (
          <div className="text-center py-12">
